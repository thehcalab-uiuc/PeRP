...
class FFN(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config.setdefaults(layers=[64, 'tanh', 64, 'tanh'], weight_scale='default', weight_init='orthogonal')
        layers = config.layers
        if isinstance(layers, list):
            layers = Namespace(s=[], v=layers, p=layers)
        s_sizes = [config.observation_space.shape[0], *layers.s]

        self.shared = build_fc(*s_sizes)

        self.p_head = build_fc(s_sizes[-1], *layers.p, config.model_output_size)
    ...
    def sequential_init(self, seq, key):
        ...
        for i, m in enumerate(linears):
            if isinstance(config.weight_scale, (int, float)):
                scale = config.weight_scale
            elif isinstance(config.weight_scale, (list, tuple)):
                scale = config.weight_scale[i]
            elif isinstance(config.weight_scale, (dict)):
                scale = config.weight_scale[key][i]
            else:
                scale = 0.01 if m == linears[-1] else 1
            if config.weight_init == 'normc': # normalize along input dimension
                weight = torch.randn_like(m.weight)
                m.weight.data = weight * scale / weight.norm(dim=1, keepdim=True)
            elif config.weight_init == 'orthogonal':
    ...
    def forward(self, inp, value=False, policy=False, argmax=None):
        ...
        if policy or argmax is not None:
            ...
            if argmax is not None:
                dist = self.config.dist_class(pred.policy)
